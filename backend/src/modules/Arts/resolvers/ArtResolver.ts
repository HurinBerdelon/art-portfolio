...
export class ArtResolver {
  ...
  @Query(() => Number)
  async numberOfArts(@Arg("categoryTitle") categoryTitle: string) {
  ...
  @Query(() => [Art])
  async artsPaginated(@Arg("skip") skip: number, @Arg("take") take: number) {
    const findPaginatedArtsUseCase = container.resolve(
      FindPaginatedArtsUseCase
    );
  ...
  @Query(() => Art)
  async artById(@Arg("id") id: string) {
  ...
  @Query(() => Boolean)
  async deleteArt(@Arg("id") id: string) {
